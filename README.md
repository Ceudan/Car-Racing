...
![Policy Value Equation](images/Policy_Value_Equation.png)

Policy Gradient methods sample actions according to a probability distribution 𝜋(a|s). The parameters of the probability distribution can be output by a neural network. We can define our objective as maximizing the value J(θ), which is the expected discounted reward from starting off in a state V(s), weighted over a particlar state distribution μ(s). To get the derivative of J(θ) w.r.t. our network weights we first replace V(s) with its equivalent ΣQ(s,a)𝜋(a|s). 𝜋(a|s) is given by our network therefore including it in our loss function allows the optimizer to adjust the necessary parameters. Notice that it seems that increasing the probabiity of all actions is the correct solution. In reality, the 𝜋(a|s) probabilities for a state must add to one, so the Q(s,a) weighting factor will naturally increase and decrease the appropriate probabilites. The authors chose μ(s) to be the natural distribution of time spent per state by the current policy. Therefore the gradient update per sampled experience obeys μ(s) in expectation, though it is a noisy approximation. Additionally we only sample a particular action per state visit instead of the summation, so we introduce the denominator to reweigh disproportionally sampled actions. Finally, we can replace Q(s,a) with real sampled reward returns Gt.
